...
//     int height;
//     int components
//     uint8_t* pImageData = drpcx_load_file("my_image.pcx", DRPCX_FALSE, &width, &height, &components, 0);
//     if (pImageData == NULL) {
...
uint8_t* drpcx_load(drpcx_read_proc onRead, void* pUserData, drpcx_bool32 flipped, int* x, int* y, int* internalComponents, int desiredComponents);
...
uint8_t* drpcx_load_file(const char* filename, drpcx_bool32 flipped, int* x, int* y, int* internalComponents, int desiredComponents);
#endif

// Helper for loading an PCX file from a block of memory.
uint8_t* drpcx_load_memory(const void* data, size_t dataSize, drpcx_bool32 flipped, int* x, int* y, int* internalComponents, int desiredComponents);
...
uint8_t* drpcx_load_file(const char* filename, drpcx_bool32 flipped, int* x, int* y, int* internalComponents, int desiredComponents)
{
    ...
    uint8_t* pImageData = drpcx_load(drpcx__on_read_stdio, pFile, flipped, x, y, internalComponents, desiredComponents);
...
static uint8_t* drpcx__row_ptr(drpcx* pPCX, drpcx_uint32 row)
{
...
uint8_t* drpcx_load(drpcx_read_proc onRead, void* pUserData, drpcx_bool32 flipped, int* x, int* y, int* internalComponents, int desiredComponents)
{
    ...
    if (desiredComponents != (int)pcx.components) {
        uint8_t* pNewImageData = (uint8_t*)malloc(pcx.width * pcx.height * desiredComponents);
        if (pNewImageData == NULL) {
            free(pcx.pImageData);
            return NULL;
        }

        uint8_t* pSrcData = pcx.pImageData;
        uint8_t* pDstData = pNewImageData;
        if (desiredComponents < (int)pcx.components) {
